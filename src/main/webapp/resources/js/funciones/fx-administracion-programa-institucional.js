...
function obtenerProgramaInstitucional(idProgramaInstitucional){
	$.ajax({
        ...
        error: function() {
            swal("ERROR", "Se ha perdido la comunicaci贸n con el servidor 贸 el recurso que busca ya no existe!, intentelo mas tarde.");
        }
    });
}
...
function capturaProgramaInstitucional(){
	...
	if(validarCamposProgramaInstitucional(programaInstitucional)){
		$.ajax({
	        ...
	        error: function() {
	            swal("ERROR", "Se ha perdido la comunicaci贸n con el servidor 贸 el recurso que busca ya no existe" +
	            		" 贸 la CLAVE del programa institucional esta duplicada y se evito la actualizaci贸n!, intentelo mas tarde.");
	        }
	    });
	}
}
...
function validarCamposProgramaInstitucional({idProgramaInstitucional = "", clave = "", nombre = "", descripcion = ""}){
	if(idProgramaInstitucional === ''){
		...
		swal("ERROR", "La descripci贸n del programa institucional no puede superar los 1200 caracteres");
		return false;
	}else{
		return true;
	}
...
function bajaProgramaInstitucional(idProgramaInstitucional) {
    swal({
        title: "縀st醩 realmente seguro de borrar este Programa Institucional?",
        text: "Se borrara este programa institucional permanentemente con todos sus 贸rganos de direcci贸n estrat茅gica y unidades de apoyo asociadas!",
        type: "warning",
