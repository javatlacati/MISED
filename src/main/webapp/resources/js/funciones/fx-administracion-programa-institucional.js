...
function obtenerProgramaInstitucional(idProgramaInstitucional){
	$.ajax({
        ...
        error: function() {
            swal("ERROR", "Se ha perdido la comunicaci�n con el servidor � el recurso que busca ya no existe!, intentelo mas tarde.");
        }
    });
}
		...
		$.ajax({
	        ...
	        error: function() {
	            swal("ERROR", "Se ha perdido la comunicación con el servidor ó el recurso que busca ya no existe" +
	            		" ó la CLAVE del programa institucional esta duplicada y se evito la actualización!, intentelo mas tarde.");
	        }
	    });
	}
}
